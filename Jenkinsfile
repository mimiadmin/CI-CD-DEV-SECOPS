...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'main', url: 'https://github.com/mimiadmin/CI-CD-DEV-SECOPS.git'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit mimijoso/reddit:latest "
                       sh "docker push mimijoso/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image mimijoso/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.17.60.165 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://18.118.22.117:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#dev-secops-cicd-mimitos', //update and provide your channel name
